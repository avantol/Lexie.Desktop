...
public class WsjtxParser
{
    ...
    private StatusMessage ParseStatus(string? clientId, uint schema)
    {
        ...
        // Parse optional fields for grid squares and own callsign
        string? deCall = null;
        string? deGrid = null;
        string? dxGrid = null;
        try
        {
            if (Remaining >= 4)
            {
                ReadUInt32();              // rxDF
                ReadUInt32();              // txDF
                deCall = ReadUtf8String(); // deCall
                deGrid = ReadUtf8String(); // deGrid
                dxGrid = ReadUtf8String(); // dxGrid
            }
        }
        catch { /* older WSJT-X versions may not send these */ }

        return new StatusMessage(clientId, schema, dialFreq, mode, dxCall, report,
            txMode, txEnabled, transmitting, decoding, deCall, deGrid, dxGrid);
    }
